...
export default function KycPage() {
  ...
  const getStatusMessage = (status: string | null) => {
    switch (status) {
      case 'approved':
        return 'Your identity has been verified and approved.';
      case 'pending':
        return 'Your KYC submission is under review. This typically takes 1-2 business days.';
      case 'rejected':
        return 'Your KYC submission was rejected. Please review the reasons and resubmit.';
      default:
        return 'Complete KYC verification to access all Kingdom features.';
    }
  };
        ...
        {isLoading ? (
          ...
          <>
              ...
              <CardContent className="space-y-4">
                ...
                {kycStatus?.submittedAt && (
                  ...
                  >
                    ...
                    <span>
                      Submitted:{' '}
                      {new Date(kycStatus.submittedAt as string | number | Date).toLocaleString()}
                    </span>
                  </div>
                )}
                ...
                {kycStatus?.reviewedAt && (
                  <div
                    className="flex items-center gap-2 text-sm text-muted-foreground"
                    data-testid="text-reviewed"
                  >
                    <CheckCircle className="h-4 w-4" />
                    <span>
                      Reviewed:{' '}
                      {new Date(kycStatus.reviewedAt as string | number | Date).toLocaleString()}
                    </span>
                  </div>
                )}
              </CardContent>
            ...
            {kycStatus?.reviewResult &&
              typeof kycStatus.reviewResult === 'object' &&
              Object.keys(kycStatus.reviewResult as Record<string, any>).length > 0 && (
                <Card data-testid="card-review-result">
                  ...
                  <CardContent>
                    <pre
                      ...
                      data-testid="text-review-result"
                    >
              ...
              <Alert data-testid="alert-no-kyc">
                ...
                <AlertDescription>
                  <div className="space-y-2">
                    ...
                    <p className="text-sm">
                      Complete KYC verification to unlock trading, payments, and advanced Kingdom
                      features. Click "Submit KYC" above to get started.
                    </p>
                  </div>
                </AlertDescription>
              </Alert>
